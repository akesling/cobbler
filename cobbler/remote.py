...
class CobblerXMLRPCInterface:
    ...
    def _build_interface(self, objs=None, interface=None):
        ...
        method to create the specific method.
        ...
        """
        default_objs = objs or ['distro', 'profile', 'system', 'repo', 'image']
        ...
        for meth, types in interface.items():
            base_method = getattr(self, "_".join((meth, "item")))
            
            for obj_type in types:
                new_method_name = "_".join((meth, obj_type))
                new_method = curry(base_method, obj_type)
